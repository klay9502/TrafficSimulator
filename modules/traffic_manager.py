...
class TrafficManager:
    def __init__(self, env, conf, pattern, vm) -> None:
        self.env = env
        self.conf = conf
        ...
        while True:
            ...
            if self.conf.learningType == "BASIC_Q":
                ...
                if np.random.rand() < self.conf.epsilon:
                    ...
                    self.algorithm.update_table(self.pattern.nowTime, self.nowSignal, self.vm.get_now_discomfort_value())
                    self.nowSignal = self.algorithm.get_action(self.pattern.nowTime, self.vm.get_now_discomfort_value())
